...
contract GameLogic is AccessControl {
  ...
  event NewPost(string title, string uri);
  event NewCommunity(
    uint site,
    string name,
    string rules,
  ...
  function createCommunity(
    ...
    emit NewCommunity(site, name, rules, restrictPosting, restrictVoting, sponsorPosts, gateAddress);
  }

  function verifyUserWorldcoinOrb(address user) public {
  ...
  function verifyUserWorldcoinPhone(address user) public {
    //require(tx comes from user)
  ...
  function verifyUserSismo(address user) public {
    //requrire(tx comes from user)
  ...
  function hasVerifiedPersonhood(address user) public view returns (bool) {
  ...
  function hasVerifiedAge(address user) public view returns (bool) {
    User storage userStruct = users[user];
    return userStruct.hasVerifiedPolygon13 || userStruct.hasVerifiedPolygon18;
  }

  function airdropInitialTokens(address user) private {
  ...
  function setUserDefaultVoteWeight(address user, uint8 defaultVoteWeight) public {
    //requrire(tx comes from user)
    emit DefaultVoteWeightChanged(user, defaultVoteWeight);
    users[user].defaultVoteWeight = defaultVoteWeight;
  }

  function setUserDefaultDonate(address user, bool donateWinningsByDefault) public {
    //requrire(tx comes from user)
  ...
  function createPost(
    ...
    PostToken(postTokenAddress).makeNew(isTopLevel, parentId, title, contentURI, msg.sender);
